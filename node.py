...
class Node:
	...
	def processInput(self, inputJson):
		...
		if inputJson["action"] == "createRing":
		    ...
		    outputJson = self.setConsistentHashingRing(inputJson["ring"], inputJson["ringSize"])

		elif inputJson["action"] == "get":
		    outputJson = self.getValueFromKey(inputJson["key"])

		elif inputJson["action"] == "set" :
		    outputJson = self.setValueForKey(inputJson["key"], inputJson["value"])
	...
	def setConsistentHashingRing(self, newChr, newChrSize):
		outputJson = self.consistentHashingRing.setConsistentHashingRing(newChr, newChrSize)
		outputJson["status"] = "Success"
		return outputJson 

	def getValueFromKey(self, key):
		nodeWhereTheKeyResides = self.consistentHashingRing.getServerPortFromKey(key)
		outputJson = {}
		if nodeWhereTheKeyResides == self.port:
			outputJson = self.localCache.localFetch(key)
			outputJson["fetchNode"] = self.port
		else:
			outputJson = self.remoteGetValueFromKey(key, nodeWhereTheKeyResides)
	...
	def setValueForKey(self, key, value):
		...
		if nodeWhereTheKeyResides == self.port:
			...
			outputJson = self.remotesetValueForKey(key, value, nodeWhereTheKeyResides)
